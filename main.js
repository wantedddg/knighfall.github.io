// ============================================================
// Knightfall Ember War — Single Player
// ============================================================
...
// ---- Background Images ----
const backImg = new Image(); backImg.src = "assets/rendering/back.png";
const middleImg = new Image(); middleImg.src = "assets/rendering/middle.png";
const tilesImg = new Image(); tilesImg.src = "assets/rendering/tiles.png";
...
window.addEventListener("keydown", (e) => {
  ...
  if (key === "f3") { showDebug = !showDebug; e.preventDefault(); }
});
...
canvas.addEventListener("click", () => {
  if (!isAttacking && !isJumping) {
    ...
    setTimeout(() => {
      ...
      const orcCenterX = orc.x + (orc.animConfig.frameWidth * orc.animConfig.scale) / 2;
...
function drawOrc() {
  ...
  ctx.save();
  const centerX = orc.x - cameraX + (orc.animConfig.frameWidth * orc.animConfig.scale) / 2;
  const centerY = orc.y + (orc.animConfig.frameHeight * orc.animConfig.scale) / 2;
  ctx.translate(centerX, centerY);
  if (!orc.facingRight) ctx.scale(-1, 1);

  ctx.drawImage(
    img,
    orc.currentFrame * orc.animConfig.frameWidth, 0,
    orc.animConfig.frameWidth, orc.animConfig.frameHeight,
  ...
  if (!orcIsDead) {
    const barWidth = 80;
    const barHeight = 18;
    const barX = orc.x - cameraX + (orc.animConfig.frameWidth * orc.animConfig.scale) / 2 - barWidth / 2 + 5;
...
function gameLoop() {
  ...
  if (now - debugLastFpsTime >= 1000) { debugFps = debugFrameCount; debugFrameCount = 0; debugLastFpsTime = now; }

  // === BACKGROUNDS ===
  const bScale = 3;
  const backWidth = backImg.width * bScale, backHeight = backImg.height * bScale;
  ...
  const mScale = 3;
  const middleWidth = middleImg.width * mScale, middleHeight = middleImg.height * mScale;
  const middleY2 = canvas.height - middleHeight - 60;
  for (let x = 0; x < worldWidth; x += middleWidth)
    ctx.drawImage(middleImg, x - cameraX * 0.5, middleY2, middleWidth, middleHeight);

  const ts = tileBaseSize * tileScale + 50;
  const groundTileY = canvas.height - ts * 1.6;
  for (let x = 0; x < worldWidth; x += ts)
    ctx.drawImage(tilesImg, 16, 16, tileBaseSize, tileBaseSize, x - cameraX, groundTileY, ts, ts);

  // === HUD ===
  ctx.fillStyle = "white"; ctx.font = "20px 'Pixelify Sans'";
  ctx.fillText("SCORE: " + score, 20, 40);
  ctx.fillText("HP: " + playerHealth, 20, 70);

  // === PLAYER MOVEMENT ===
  let moving = false;
  if (keys.a) { charX -= speed; moving = true; facingRight = false; }
  if (keys.d) { charX += speed; moving = true; facingRight = true; }
  if (keys.shift) { speed = defaultSpeed + 1.1; } else { speed = defaultSpeed; }
  ...
  if (isJumping) {
    charY += jumpVel;
    jumpVel += gravity;
    if (charY >= charBaseY) { charY = charBaseY; isJumping = false; jumpVel = 0; }
  }
      ...
      if (absDistance < orcFollowRange) {
        ...
        if (absDistance > orcAttackRange) {
          ...
          setTimeout(() => {
            ...
            const playerCenterXNow = charX + (animConfig.frameWidth * animConfig.scale) / 2;
            const orcCenterXNow = orc.x + (orc.animConfig.frameWidth * orc.animConfig.scale) / 2;
            ...
            if (impactDistance <= orcAttackRange) {
              if (!isHurt) {
                ...
                switchAnim("hurt");
                setTimeout(() => { isHurt = false; }, 400);
              }
            }
          }, 300);

          setTimeout(() => { orcIsAttacking = false; }, 1000);
        }
      } else {
  ...
  if (isAttacking && currentFrame >= animConfig.totalFrames - 1) { isAttacking = false; }

  // === DRAW LOCAL PLAYER ===
  const img = images[currentAnim];
  ctx.save();
  const centerX = charX - cameraX + (animConfig.frameWidth * animConfig.scale) / 2;
  const centerY = charY + (animConfig.frameHeight * animConfig.scale) / 2;
  ctx.translate(centerX, centerY);
  if (!facingRight) ctx.scale(-1, 1);
  ctx.drawImage(img,
    currentFrame * animConfig.frameWidth, 0, animConfig.frameWidth, animConfig.frameHeight,
    -(animConfig.frameWidth * animConfig.scale) / 2, -(animConfig.frameHeight * animConfig.scale) / 2,
    animConfig.frameWidth * animConfig.scale, animConfig.frameHeight * animConfig.scale);
  ctx.restore();

  // === FRAME COUNTER ===
  let actualFrameSpeed = animConfig.frameSpeed;
  if (currentAnim === "walk" && keys.shift) actualFrameSpeed = Math.max(1, animConfig.frameSpeed / 2);
  frameCounter++;
  if (frameCounter >= actualFrameSpeed) { frameCounter = 0; currentFrame++; if (currentFrame >= animConfig.totalFrames) currentFrame = 0; }
...
function drawDebug() {
  ...
  const pad = 10, lH = 18, pW = 200, pH = lines.length * lH + pad * 2;
  const pX = canvas.width - pW - 10, pY = 10;
  ctx.fillStyle = "rgba(0,0,0,0.65)"; ctx.fillRect(pX, pY, pW, pH);
  ctx.strokeStyle = "rgba(255,255,255,0.3)"; ctx.lineWidth = 1; ctx.strokeRect(pX, pY, pW, pH);
  ctx.fillStyle = "#00ff88"; ctx.font = "13px 'Pixelify Sans'"; ctx.textAlign = "left"; ctx.textBaseline = "top";
  for (let i = 0; i < lines.length; i++) ctx.fillText(lines[i], pX + pad, pY + pad + i * lH);
  ctx.textBaseline = "alphabetic";
}

// ============= AMBIENT MUSIC (Minecraft-style) =============
const bgMusic = new Audio("assets/sounds/Brighter_Music.m4a");
bgMusic.loop = false; // never loop — we control replay manually
bgMusic.volume = 0.1;

const MUSIC_MIN_DELAY = 30;  // minimum seconds between plays
const MUSIC_MAX_DELAY = 120; // maximum seconds between plays

let musicUnlocked = false; // browser autoplay gate
...
function scheduleNextPlay() {
  const delaySec = MUSIC_MIN_DELAY + Math.random() * (MUSIC_MAX_DELAY - MUSIC_MIN_DELAY);
  setTimeout(playAmbientMusic, delaySec * 1000);
}

// When the track ends, wait a random delay then play again
bgMusic.addEventListener("ended", scheduleNextPlay);

// Unlock audio on first user interaction, then kick off the 5s initial delay
function unlockMusic() {
  if (musicUnlocked) return;
  musicUnlocked = true;
  // First play after 5 seconds
